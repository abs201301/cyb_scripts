...
try {
   $profiles = Get-WmiObject Win32_UserProfile |
       Where-Object { $_.LocalPath -match '\\Users\\PSM-' -and -not $_.Special }
   foreach ($profile in $profiles) {
       $userProfilePath = $profile.LocalPath
       $sid = $profile.SID
       $userName = $userProfilePath -replace 'C:\\Users\\', ''
       if ($profile.Loaded) {
           Add-Content -Path $logpath -Value ((Get-Date -Format "yyyy/MM/dd HH:mm") + " Profile in use, skipping: $userName")
           continue
       }
