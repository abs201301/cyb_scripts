...
Func UpdateWebDriver($sBrowser)
   ...
   If FileExists($sDriverFile) Then
       ...
       If StringLeft($sDrvVer, StringLen($sMajorVer)) = $sMajorVer Then
           LogWrite($sBrowser & " webdriver already matches major version " & $sMajorVer)
           Return
       EndIf
   EndIf
   ...
   If $sBrowser = "GoogleChrome" Then
       ...
       Local $sApi = "https://chromedriver.storage.googleapis.com/LATEST_RELEASE_" & $sMajorVer
       Local $sVerFile = $g_sDriverPath & "chrome_latest.txt"
       InetGet($sApi, $sVerFile, $INET_FORCERELOAD, $INET_DOWNLOADWAIT)
       $sDriverVer = StringStripWS(FileRead($sVerFile), 3)
       FileDelete($sVerFile)
       If $sDriverVer = "" Then
           LogWrite($MB_ICONERROR & "Error- Could not fetch ChromeDriver version")
           Return
       EndIf

       $sUrl = "https://chromedriver.storage.googleapis.com/" & $sDriverVer & "/chromedriver_win32.zip"
   ElseIf $sBrowser = "edge" Then

       Local $sApi = "https://msedgedriver.azureedge.net/LATEST_RELEASE_" & $sMajorVer
       Local $sVerFile = $g_sDriverPath & "edge_latest.txt"
       InetGet($sApi, $sVerFile, $INET_FORCERELOAD, $INET_DOWNLOADWAIT)
       $sDriverVer = StringStripWS(FileRead($sVerFile), 3)
       FileDelete($sVerFile)
       If $sDriverVer = "" Then
           LogWrite($MB_ICONERROR & "Error- Could not fetch EdgeDriver version")
           Return
       EndIf
       If $sBit = "64" Then
   ...
   If Not FileExists($sZip) Then
       LogWrite($MB_ICONERROR & "Error- Failed to download " & $sBrowser & " driver")
       Return
   EndIf
...
; =================== Write Log messages ===================
Func LogWrite($lMsg)
	_FileWriteLog($g_sDriverPath & "\WebDriverUpdater.log", $lMsg)
EndFunc
